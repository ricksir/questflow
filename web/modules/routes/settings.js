...
function element(id) {
    ...
    if (!node)
        throw new Error(`Controle ausente: ${id}`);
    return node;
}
function setStatus(message, ok = null) {
    element('questionSourceStatusText').textContent = message;
    const pill = element('questionSourceStatusPill');
...
async function loadSettings() {
    try {
        const settings = await studioGet('question-source');
        element('questionSourceProvider').value = settings.provider;
        element('apiDasQuestoesBaseUrl').value = settings.api_das_questoes.base_url;
        element('apiDasQuestoesTimeout').value = String(settings.api_das_questoes.timeout_seconds);
        element('apiDasQuestoesKeyHint').textContent = settings.api_das_questoes.api_key_configured
...
async function saveSettings() {
    ...
    try {
        const settings = await studioPost('question-source', {
            provider: element('questionSourceProvider').value,
            base_url: element('apiDasQuestoesBaseUrl').value.trim(),
            timeout_seconds: Number(element('apiDasQuestoesTimeout').value || 20),
            api_key: element('apiDasQuestoesKey').value.trim(),
        });
        element('apiDasQuestoesKey').value = '';
        element('apiDasQuestoesKeyHint').textContent = settings.api_das_questoes.api_key_configured
            ? 'Chave configurada e protegida. Deixe o campo vazio para mantê-la.'
            : 'Ainda não configurada.';
...
async function testSource() {
    ...
    try {
        const result = await studioPost('question-source/test', {
            provider: element('questionSourceProvider').value,
        });
...
const module = {
    async mount({ page }) {
        if (!document.getElementById('questionSourceSettingsPanel')) {
            ...
            const network = page.querySelector('.network-panel');
            if (!grid)
                return;
            if (network)
                network.insertAdjacentHTML('beforebegin', panelHtml());
            else
                grid.insertAdjacentHTML('beforeend', panelHtml());
            element('saveQuestionSource').addEventListener('click', () => void saveSettings());
            element('testQuestionSource').addEventListener('click', () => void testSource());
        }
        if (!mounted) {
            mounted = true;
            await loadSettings();
