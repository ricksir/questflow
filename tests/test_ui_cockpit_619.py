...
class QuestFlowCockpit619Tests(unittest.TestCase):
    def test_studio_exposes_learning_pulse(self):
        ...
        self.assertIn("const visibleFocus = focusSubjects.slice(0, 6)", script)
        self.assertIn("hiddenFocusCount", script)
        self.assertIn("data-pulse-all", script)
        self.assertIn("data-pulse-subject-key", script)
        ...
        self.assertIn(".learning-focus-grid", styles)
    ...
    def test_mobile_uses_shared_pulse_components(self):
        ...
        self.assertIn("visibleSubjectCount", progress)
        self.assertIn("Mostrar mais", progress)
        self.assertIn("ROTEIRO DA SESSÃO", today)
        self.assertIn("Do reforço à consolidação", today)
