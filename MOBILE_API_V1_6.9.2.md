# QuestFlow Mobile API v1 — Studio 6.9.2 / Mobile 0.1.1

A 6.9.2 preserva o contrato `questflow.mobile.v1` criado na 6.9.0 e acrescenta o primeiro cliente móvel funcional. O aplicativo consome projeções próprias; ele **não acessa diretamente** `study_state`, tentativas, FSRS, KT, IRT ou outras tabelas internas do Studio.
...
2. Abra **Configurações → QuestFlow Mobile 0.1.1 — Rede dinâmica integrado ao Studio 6.9.2**.
...
## Rede local dinâmica — 6.9.2

O Studio recalcula `api_base_urls` no momento do pareamento, priorizando o IPv4 da rota padrão. O QR pode transportar múltiplos parâmetros `server=`. O Mobile 0.1.1 persiste candidatos conhecidos e, se o endereço salvo deixar de responder após troca de Wi-Fi/LAN, faz descoberta controlada na /24 atual. Após o pareamento, a redescoberta compara `server_fingerprint` antes de enviar o Bearer token ao endpoint candidato.
